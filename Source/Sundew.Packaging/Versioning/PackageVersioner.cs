// --------------------------------------------------------------------------------------------------------------------
// <copyright file="PackageVersioner.cs" company="Sundews">
...
public class PackageVersioner : IPackageVersioner
{
        ...
        string versionMetadata)
    {
        ...
        if (selectedSource.IsStableRelease)
        {
            return new NuGetVersion(latestVersion.Major, latestVersion.Minor, latestVersion.Patch, latestVersion.Revision + revisionIncrement, default(IEnumerable<string>), versionMetadata);
        }
    ...
    private NuGetVersion GetNoChangeVersion(DateTime buildDateTime, NuGetVersion nugetVersion, SelectedStage selectedSource, string metadata, string parameter, string versionMetadata)
    {
        if (selectedSource.IsStableRelease)
        {
            return new NuGetVersion(nugetVersion.Major, nugetVersion.Minor, nugetVersion.Patch, nugetVersion.Revision, default(IEnumerable<string>), versionMetadata);
        }
    ...
    {
        if (!metadataFormat.IsNullOrEmpty())
        {
